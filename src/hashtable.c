...
static void h_init(HTab *htab, size_t n, Arena *arena) {
  assert(n > 0 && ((n - 1) & n) == 0);
  htab->tab = (HNode **)make(arena, HNode *, n);
  htab->mask = n - 1;
  htab->size = 0;
}
...
static void hm_start_resizing(HMap *hmap, Arena *arena) {
  assert(hmap->ht2.tab == NULL);
  // create a bigger hashtable and swap them
  hmap->ht2 = hmap->ht1;
  h_init(&hmap->ht1, (hmap->ht1.mask + 1) * 2, arena);
  hmap->resizing_pos = 0;
}
...
HNode *hm_pop(HMap *hmap, HNode *key, bool (*eq)(HNode *, HNode *)) {
  hm_help_resizing(hmap);
  HNode **from;
  if ((from = h_lookup(&hmap->ht1, key, eq))) {
    return h_detach(&hmap->ht1, from);
  }
  if ((from = h_lookup(&hmap->ht2, key, eq))) {
